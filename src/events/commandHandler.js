  ...
  if (cmd) {
    ...
    if (cd) {
      ...
      if (diff < cooldown)
        if (!sended) {
          sended = true;
          return message.channel.send(`To be able to use this command again you must wait for **${Number(((cooldown - diff) / 1000).toFixed(2))}**!`).then((x) => x.delete({ timeout: (cooldown - diff) }));
        }
